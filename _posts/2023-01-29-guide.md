...
With this lab, you will be walked through the steps to migrate a Wi-Fi 6E AP from management with the 9800 Wireless LAN Controller to the Meraki cloud and vice versa, and to demonstrate Wi-Fi 6E configuration.
...
| **Pod**                  | **User**            | **Password**  |
| ------------------------ |:-------------------:| -------------:|
| 1  Andrius Ajauskas      | `v926user1   `      | `b4a241`      |   
| 2  Simon Balon           | `v1103user1 `       | `af7bb4`      |   
| 3  Keith Clarke          | `v293user1  `       | `c76ca5`      |   
| 4  Amanuel Getaneh	   | `v1881user1 `       | `c11a62`      |   
| 5  Christian Ginser	   | `v2139user1 `       | `bf9e68`      |   
| 6  Rakesh Kapoor         | `v648user1  `       | `cf8d63`      |   
| 7  Ole Kasten Bechtle    | `v288user1  `       | `822aba`      |   
| 8  grzegorz Koziel	   | `v899user1  `       | `614897`      |   
| 9  Rene Mueller          | `v1340user1 `       | `5c906a`      |   
| 10 Heriel Onaisrael	   | `v1958user1 `       | `306789`      |   
| 11 Frank Osberg          | `v2237user1 `       | `23309e`      |   
| 12 Ivo Peeters           | `v1413user1 `       | `a8cb4b`      |   
| 13 Pim Servaas           | `v419user1  `       | `a03fbc`      |   
| 14 Andrei Sidorkov	   | `v1996user1 `       | `ea663e`      |   
| 15 Bjorn Svensson        | `v1312user1 `       | `3592e7`      |   
| 16 Bernd Syrowatka	   | `v2576user1 `       | `9a0d8c`      |   
```
...
13. Test connectivity by pinguing the controller default gateway. 

```
C9800# ping 198.19.10.254
```

If you encounter any issues please let us know.

14.Access via GUI. In your remote desktop open a browser and type **https://100.64.0.7** or click on the **C9800-CL** bookmark.
...
In the Wireless Network Settings click  **+Add**,
...
5.Test connectivity by pinging the controller default gateway, for this please open the WLC CLI using mRemoteNG App.
...
| **Pod**                  | *Public IP*         |
| ------------------------ |:-------------------:|
| 1  Andrius Ajauskas      | `64.100.10.198`     |
| 2  Simon Balon           | `64.100.10.239`     |
| 3  Keith Clarke          | `64.100.10.150`     | 
| 4  Amanuel Getaneh	   | `64.100.10.99`      |
| 5  Christian Ginser	   | `64.100.10.238`     |
| 6  Rakesh Kapoor         | `64.100.10.217`     | 
| 7  Ole Kasten Bechtle    | `64.100.10.191`     | 
| 8  grzegorz Koziel	   | `64.100.10.206`     | 
| 9  Rene Mueller          | `64.100.10.211`     | 
| 10 Heriel Onaisrael	   | `64.100.10.237`     | 
| 11 Frank Osberg          | `64.100.10.180`     | 
| 12 Ivo Peeters           | `64.100.10.236`     | 
| 13 Pim Servaas           | `64.100.10.199`     |
| 14 Andrei Sidorkov	   | `64.100.10.233`     |
| 15 Bjorn Svensson        | `64.100.10.223`     |
| 16 Bernd Syrowatka	   | `64.100.10.227`     |
```

Select your corresponding Public IP address from above table, then navigate to **Configuration > Interface > Wireless**, click on the Management interface and add the public IP address to the "NAT IPv4" field as in below image, then hit "**Update & Apply to Device**".

![](/images/wmi-nat.png)

Once the Public IP is configured you will see your assigned Access Point joining to your brand new 9800 Controller.

This step can take around 5 minutes. In the meantime, let's see the C9800 Configuration model and configure the tags, we will come back to check the AP has joined in few minutes.
...
| **Pod**                  | PID         | Country  |
| ------------------------ | ----------- | ---------|
| 1  Andrius Ajauskas      | CW9166I-A   | CA       |
| 2  Simon Balon           | CW9166I-A   | CA       |
| 3  Keith Clarke          | CW9164I-A   | CA       |
| 4  Amanuel Getaneh	   | CW9164I-A   | CA       |
| 5  Christian Ginser	   | CW9164I-A   | CA       |
| 6  Rakesh Kapoor         | CW9164I-A   | CA       |
| 7  Ole Kasten Bechtle    | CW9166I-A   | CA       |
| 8  grzegorz Koziel	   | CW9166I-A   | CA       |
| 9  Rene Mueller          | CW9164I-A   | CA       |
| 10 Heriel Onaisrael	   | CW9164I-MR  | US       |
| 11 Frank Osberg          | CW9164I-A   | CA       |
| 12 Ivo Peeters           | CW9166I-MR  | US       |
| 13 Pim Servaas           | CW9166I-MR  | US       |
| 14 Andrei Sidorkov	   | CW9166I-MR  | US       |
| 15 Bjorn Svensson        | CW9166I-A   | CA       |
| 16 Bernd Syrowatka	   | CW9166I-A   | CA       |
```
...
Verify your AP joined the controller using the "**show ap summary**" command. 
...
Observe that when a brand new Access Point joins, the controller assigns the default tags. 
...
To check the current tags click on the AP entry and look at the "Tags" section in the "General" tab. Observe that when a brand new Access Point joins, the Controller assigns the default tags. 
...
The change of Enterprise Access Points to Meraki Persona can be triggered from the Wireless LAN Controller only after the AP has successfully joined to the WLC. 
...
The conversion can be triggered per Access Point or to multiple Access Points at once. Choose whatever method you feel more comfortable with. 
...
2.Validate the country code check is green, and regulatory domains for Slot 0 and Slot 1 are green too. It is possible that Slot 2 corresponding to 6GHz is red due to country configured is not supported in current version, however the conversion can be triggered. Click **Next**.
...
Review if there are failed conversions, issue the "**show ap management-mode meraki failure summary**" command from CLI.
...
_- Device entitled with DNA Essentials will receive a Meraki enterprise Level License_
...
Note that in order to claim the Access Points, you must have the appropriate licenses, such as co-termination or per-device licensing models. On the reference section we leave the links to understand the license model. 

### Troubleshooting tools

Troubleshooting tools in the 9800 are quite powerful. To know in detail about the 9800 troubleshooting tools we encourage you to assist the "_Troubleshoot Catalyst 9800 Wireless Controllers - BRKEWN-3628_" (taking place Friday 9:00 am). During this section we will cover an overview of the most used tools. 

**Syslog**

This presents the logs generated by the 9800 WLC, this output shows general logs as well as some wireless-specifics logs. 

Verify that AP was successfully converted from controller syslog. Look for the log "**Meraki persona change result: Success. Terminating the CAPWAP session.**" followed by the AP disjoin log.
...
Navigate to **Troubleshooting > Radioactive Trace** . You will find the RadioActive trace we enabled before (in the AP join section) filtered by Radio and Ethernet MAC. Select one of them and click "Generate", generate logs for last 30 minutes and "Apply to Device"
...
**_For this LAB we have created the users under the same account, as per devices register under the same license._**
...
_Meraki offers free trials so it is very to play with your own account_ 
...
```
| **Pod**                  | PID         | Serial Number Meraki  |
| ------------------------ | ----------- | ----------------------|
| 1  Andrius Ajauskas      | CW9166I-A   | Q5AF-K93M-5QSE        |
| 2  Simon Balon           | CW9166I-A   | Q5AF-XXW4-XB2R        |
| 3  Keith Clarke          | CW9164I-A   | Q5AD-VZZG-UUA2        |
| 4  Amanuel Getaneh	   | CW9164I-A   | Q5AD-YTKV-FRH3        |
| 5  Christian Ginser	   | CW9164I-A   | Q5AD-93SU-WKJ5        |
| 6  Rakesh Kapoor         | CW9164I-A   | Q5AD-7HE2-TFNS        |
| 7  Ole Kasten Bechtle    | CW9166I-A   | Q5AF-23LY-92QZ        |
| 8  grzegorz Koziel	   | CW9166I-A   | Q5AF-9VQV-3Q2B        |
| 9  Rene Mueller          | CW9164I-A   | Q5AD-SR9H-MNUM        |
| 10 Heriel Onaisrael	   | CW9164I-MR  | Q5AD-SVXT-Z9Z5        |
| 11 Frank Osberg          | CW9164I-A   | Q5AD-95LX-878G        |
| 12 Ivo Peeters           | CW9166I-MR  | Q5AF-24D6-GQMP        |
| 13 Pim Servaas           | CW9166I-MR  | Q5AF-CRZ6-C2L9        |
| 14 Andrei Sidorkov	   | CW9166I-MR  | Q5AF-MGZD-5XAL        |
| 15 Bjorn Svensson        | CW9166I-A   | Q5AF-SAX2-2S3G        |
| 16 Bernd Syrowatka	   | CW9166I-A   | Q5AF-UQDN-BKSB        |
```
...
In the following page we can place the device in a specific address , this will define the location for your AP in your network. 
...
Go to the **organization** >> **Overview**, you should see by now your network with the AP added and status green, meaning AP is online and ready to be configured. 
...
- For Advanced splash setting, let's use the following, check the image below
	- Captive portal strength: Choose type of traffic allowed before completing portal. 
...
All we need is a bit of creativity and HTML knowledge. 
...
From there we have available 2 themes (modern and fluid), which we can only modify colors as shown below. 

![](/images/splashpage1.png)

Click **Preview** to check how your new color selection looks like. Feel free to play around with it. 
...
You can play around adding more stuff if you want to. 
...
Click on **back to config** (right side of the screen) and select your new custom theme, so users will be seeing this page. On this page you can still configure the following: 
...
_Note: This will prevent for 802.11b clients to connect to the network, of course this depends of the type of devices deployed in such site, but in this case we are going to pretend those clients are not a problem anymore _
...
The advantage of this is to create smaller cells with strong signal and warranty high data rates for clients. 
...
- **Channel assignment method** click on **Change channels used by Autochannel..**

![](/images/radiosettings11.png)

Leave available just one channel of 160Mhz (8 channels) so we force AP to take this one. Here an example, once finished, click on **Done**.

![](/images/radiosettings12.png)

_Note: In a real scenario we would like to have the PSC (preferred scanning channels) available. These are the channels that client will actively scan to discover the SSID, if this channels are not available it can take really long time for clients to discover the SSIDs._

_PSC channel list: 5, 21, 37, 53, 69, 85, 101, 117, 133, 149, 165, 181, 197, 213 and 229_

_Meraki dashboard, based on the AP domain and country configured, will display what channels are available to use in this band_ 

Finally, **Save** the changes in the bottom of the page 

![](/images/radiosettings13.png)

You should be back to **Radio Setting** >> **RF profiles** and you should be able to see your new brand RF profile. 
...
At this point we might think that our _Corp_ and _Guest_ SSID are broadcasting on the 6Ghz band, but this is not true, WiFi 6E did not leave security out, so the security settings need to be configure to have an SSID broadcasted on the 6Ghz band: 
...
_Note: That's it!, all this looks very easy but in real world scenarios you must likely need transition scenarios, not all the clients will support WPA3, for more information on that, look at the reference link on top of this guide to learn more about it._
...
Meraki dashboard provides very powerful tool to monitor your RF environment. You can review this information for you access point using **Wireless** >>  **Monitor** >> **RF spectrum**
...
- We can also identify what other SSID are being broadcasted on the same location and how strong we can hear them.


The real advantage of Meraki here is the ability to do real-time spectrum analysis of the environment while serving clients. 
...
Meraki dashboard offer many stats, graphs to monitor traffic, health, and report problems, from wireless clients.  
As this LAB is not intended to be used with clients we prefer to leave you some link on the reference part to have more information on this if needed. 

### Migration from Meraki to 9800 WLC 

**IMPORTANT!!!**
Cisco Meraki has determined that customers who wish to convert their APs from Meraki to 9800 WLC will need to reach out to Meraki support to initiate the change. For purposes of this LAB this option is available under this account in the dashboard.
...
It will take few minutes for AP to reload. After that you should see the AP as disconnected from Meraki dashboard. 
...
_Note: For AP RMAs, once requesting the RMA please specified what management mode it is required so RMA can be submitted with this information and AP received is running the right image_

We appreciate you attending the LAB, please let us know that you have finish and if you have any question we are there to help you.
