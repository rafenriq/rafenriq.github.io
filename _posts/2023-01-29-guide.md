...
- For Advanced splash setting, let's use the following:
	- Captive portal strength: Chose type of traffic allowed before completing portal. Chose **Allow non-HTTP traffic prior to sign-on**
	...
	- Self-registration: Display a form on the web page to allow client to create a user, users need to be Admin approved before they can actually work in the portal. Select **Don't allow**
	- Simultaneous logins: allow to use same user/password in different devices. Click on **Allow simultaneous devices per user**
	- Controller disconnection behavior: If AP is not reaching Meraki Dashboard, what is the expected behavior. Leave **Default**
